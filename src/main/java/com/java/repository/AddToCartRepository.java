...
public interface AddToCartRepository extends JpaRepository<AddToCart, Long>{
	...
	@Query("select new com.java.dto.AddToCartDto(cart.id,cart.product_id,cart.quantity, cart.price, cart.added_date, cart.user_id, u.name, p.name, p.price, p.urlImg, p.fileId) from AddToCart cart, User u, Products p WHERE cart.user_id = :user_id and cart.user=u and cart.products=p")
	List<AddToCartDto> getCartByUserId (@Param("user_id") long user_id);

	@Query("select new com.java.dto.AddToCartDto(cart.id,cart.product_id,cart.quantity, cart.price, cart.added_date, cart.user_id, u.name, p.name, p.price,  p.urlImg, p.fileId) from AddToCart cart, User u, Products p WHERE cart.id = :id and cart.user_id = :userId and cart.user=u and cart.products=p")
